import type { MaskData, MaskMode, ElementType, ElementData, RectData, EllipseData, PathData } from '../../engine/types';
...
interface Props {
   ...
   *
   * 注：luminance 模式仅在 SVG 中完整支持（mask-type="luminance"）。
   * Canvas 2D 通过 globalCompositeOperation 模拟，效果近似但不完全等同。
   */
  maskMode?: MaskMode;
}
 ...
 * SVG 模式：创建 <mask> 元素
 */
function Mask({ id, shapeType, shapeData, maskMode = 'alpha' }: Props) {
  useRegisterNode(
    id,
    () => ({
      id,
