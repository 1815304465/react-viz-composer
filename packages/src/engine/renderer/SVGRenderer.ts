...
import {
  ...
  IDENTITY_MAT3, sortByPaintOrder, pointAttr, getPointsCount,
...
class SVGRenderer extends Renderer {
  ...
  private groupContainerMap: Map<string, SVGGElement> = new Map();
  /** 效果容器的 defs 定义元素（id → clipPath/filter/mask def） */
  private effectDefMap: Map<string, SVGElement> = new Map();
  /** points 节点 id → 子 ellipse 元素列表 */
  private pointsChildMap: Map<string, SVGEllipseElement[]> = new Map();
  /** 当前画布宽度 */
  ...
  private onGlobalMouseMove: ((e: MouseEvent) => void) | null = null;
  /** 全局 mouseup 监听器引用（用于解绑） */
  private onGlobalMouseUp: ((e: MouseEvent) => void) | null = null;
   ...
   * @returns SVG 根元素
   */
  mount(container: HTMLElement): SVGElement {
  ...
  override setViewport(v: { x: number; y: number; scale: number }): boolean {
    if (!super.setViewport(v)) return false;
    this.updateViewportGroup();
    return true;
  }
  ...
  protected startDrag(id: string, onDrag: VizDragEventHandler, onEnd: VizDragEventHandler, evt: MouseEvent): void {
    ...
    this.onGlobalMouseMove = globalMove as unknown as (e: MouseEvent) => void;
    this.onGlobalMouseUp = globalUp as unknown as (e: MouseEvent) => void;
  ...
  protected stopDrag(): void {
    ...
    if (this.onGlobalMouseMove) {
      document.removeEventListener('pointermove', this.onGlobalMouseMove as unknown as (e: PointerEvent) => void);
      this.onGlobalMouseMove = null;
    }
    if (this.onGlobalMouseUp) {
      document.removeEventListener('pointerup', this.onGlobalMouseUp as unknown as (e: PointerEvent) => void);
   ...
   * @param node 目标节点
   */
  private computeWorldMatrix(node: ElementRecord): Mat3 {
    if (!node.worldMatrixDirty) return node.worldMatrix;

    const parentMatrix = node.parent
      ? this.computeWorldMatrix(node.parent)
      : IDENTITY_MAT3 as Mat3;
    const localMatrix = transformToMatrix((node.data as { transform?: Transform }).transform);
    ...
    if (!this.model || !isElementVisible(this.model, node)) {
      ...
      if (culled) {
        ...
        if (this.elementMap.has(node.id)) {
          if (node.dirty) this.updateNodeDom(node);
        } else {
   ...
   * @param record 元素记录
   */
  private createNodeDom(record: ElementRecord): void {
    let el: SVGElement | null = null;
    switch (record.type) {
      case 'rect':
        el = document.createElementNS(NS, 'rect');
        this.applyRectAttrs(el as SVGRectElement, record.data as RectData, record);
        break;
      case 'ellipse':
        el = document.createElementNS(NS, 'ellipse');
        this.applyEllipseAttrs(el as SVGEllipseElement, record.data as EllipseData, record);
        break;
      case 'line':
        el = document.createElementNS(NS, (record.data as LineData).closed ? 'polygon' : 'polyline');
        this.applyLineAttrs(el as SVGPolylineElement, record.data as LineData, record);
        break;
      case 'path':
        el = document.createElementNS(NS, 'path');
        this.applyPathAttrs(el as SVGPathElement, record.data as PathData, record);
        break;
      case 'text':
        el = document.createElementNS(NS, 'text');
        this.applyTextAttrs(el as SVGTextElement, record.data as TextData, record);
        break;
      case 'image':
        el = document.createElementNS(NS, 'image');
        this.applyImageAttrs(el as SVGImageElement, record.data as ImageData, record);
        break;
      case 'points':
    ...
    if (record.type === 'group' || record.type === 'animation'
      ...
      this.groupContainerMap.set(record.id, el as unknown as SVGGElement);
    }
   ...
   */
  private createEffectContainer(
    record: ElementRecord,
    attrName: 'clip-path' | 'filter' | 'mask',
  ): SVGGElement {
    const defId = `${record.id}-def`;
    let defEl: SVGElement;
    ...
    this.applyGroupTransform(g, record.data as { transform?: Transform });
   ...
   * @param record 元素记录
   */
  private createPointsEl(record: ElementRecord): SVGGElement {
    const g = document.createElementNS(NS, 'g');
    g.setAttribute('data-viz-id', record.id);
    this.syncPointsChildren(g, record);
    this.pointsChildMap.set(record.id, Array.from(g.querySelectorAll('ellipse')) as SVGEllipseElement[]);
   ...
   * @param record 元素记录
   */
  private syncPointsChildren(container: SVGGElement, record: ElementRecord): void {
    const data = record.data as PointsData;
    ...
    for (let i = 0; i < n; i++) {
      let el = existing[i] as SVGEllipseElement | undefined;
      if (!el) {
        el = document.createElementNS(NS, 'ellipse');
   ...
   * @param record 元素记录
   */
  private updateNodeDom(record: ElementRecord): void {
    ...
    switch (record.type) {
      case 'rect':
        this.applyRectAttrs(el as SVGRectElement, data as unknown as RectData, record);
        break;
      case 'ellipse':
        this.applyEllipseAttrs(el as SVGEllipseElement, data as unknown as EllipseData, record);
        break;
      case 'line':
        this.applyLineAttrs(el as SVGPolylineElement, data as unknown as LineData, record);
        break;
      case 'path':
        this.applyPathAttrs(el as SVGPathElement, data as unknown as PathData, record);
        break;
      case 'text':
        this.applyTextAttrs(el as SVGTextElement, data as unknown as TextData, record);
        break;
      case 'image':
        this.applyImageAttrs(el as SVGImageElement, data as unknown as ImageData, record);
        break;
      case 'points':
        this.syncPointsChildren(el as SVGGElement, record);
        break;
      case 'group':
      case 'animation':
      case 'clipPath':
      case 'filter':
      case 'mask':
        this.applyGroupTransform(el as SVGGElement, data as { transform?: Transform });
        this.applyCommonAttrs(el, record);
        this.updateEffectDef(record);
        break;
      case 'linearGradient':
        this.updateLinearGradient(el as SVGLinearGradientElement, data as unknown as LinearGradientData);
        break;
      case 'radialGradient':
        this.updateRadialGradient(el as SVGRadialGradientElement, data as unknown as RadialGradientData);
        break;
    }
  }
   ...
   * @param record 节点
   */
  private updateEffectDef(record: ElementRecord): void {
    const defEl = this.effectDefMap.get(record.id);
    if (!defEl) return;
    const defId = `${record.id}-def`;
    if (record.type === 'clipPath') {
      this.updateClipPathEl(defEl as SVGClipPathElement, record.data as ClipPathData, defId);
    } else if (record.type === 'filter') {
      this.updateFilterEl(defEl as SVGFilterElement, record.data as FilterData, defId);
    } else if (record.type === 'mask') {
      this.updateMaskEl(defEl as SVGMaskElement, record.data as MaskData, defId);
    }
  }
   ...
   * @returns 父容器 SVG 元素
   */
  private resolveParentContainer(record: ElementRecord): SVGElement | null {
    if (record.type === 'linearGradient' || record.type === 'radialGradient') {
      return this.defs;
    }
    const parent = record.parent;
    if (!parent) return this.viewportGroup;
  ...
  private createGroupEl(record: ElementRecord): SVGGElement {
    const g = document.createElementNS(NS, 'g');
    const data = record.data as { transform?: Transform };
  ...
  private applyGroupTransform(g: SVGGElement, data: { transform?: Transform }): void {
   ...
   */
  protected drawSelf(_node: ElementRecord): void {
  ...
  renderElement(record: ElementRecord): boolean {
    ...
    this.renderNode(record, IDENTITY_MAT3 as Mat3, null);
    return true;
  }
   ...
   * @param record 元素记录
   */
  private applyRectAttrs(el: SVGRectElement, data: RectData, record: ElementRecord): void {
    ...
    if (data.rx !== undefined) el.setAttribute('rx', String(data.rx));
    else el.removeAttribute('rx');
    if (data.ry !== undefined) el.setAttribute('ry', String(data.ry));
    else el.removeAttribute('ry');
   ...
   * @param record 元素记录
   */
  private applyEllipseAttrs(el: SVGEllipseElement, data: EllipseData, record: ElementRecord): void {
   ...
   * @param record 元素记录
   */
  private applyLineAttrs(el: SVGPolylineElement, data: LineData, record: ElementRecord): void {
    el.setAttribute('points', data.points.map((p) => `${p.x},${p.y}`).join(' '));
    el.setAttribute('fill', data.closed && data.fill ? data.fill : 'none');
    if (data.stroke) el.setAttribute('stroke', data.stroke);
    if (data.strokeWidth !== undefined) el.setAttribute('stroke-width', String(data.strokeWidth));
    this.setPaintAttrs(el, data);
    this.setTransform(el, data.transform);
    this.applyCommonAttrs(el, record);
  }
   ...
   * @param record 元素记录
   */
  private applyPathAttrs(el: SVGPathElement, data: PathData, record: ElementRecord): void {
   ...
   * @param record 元素记录
   */
  private applyTextAttrs(el: SVGTextElement, data: TextData, record: ElementRecord): void {
    el.setAttribute('x', String(data.x));
    el.setAttribute('y', String(data.y));
    if (data.fontSize) el.setAttribute('font-size', String(data.fontSize));
    if (data.fontFamily) el.setAttribute('font-family', data.fontFamily);
    if (data.fontWeight !== undefined) el.setAttribute('font-weight', String(data.fontWeight));
    if (data.textAlign) el.setAttribute('text-anchor', data.textAlign);
    if (data.textBaseline) el.setAttribute('dominant-baseline', data.textBaseline);
    if (data.fill) el.setAttribute('fill', data.fill);
    if (data.stroke) el.setAttribute('stroke', data.stroke);
    if (data.strokeWidth !== undefined) el.setAttribute('stroke-width', String(data.strokeWidth));
   ...
   * @param record 元素记录
   */
  private applyImageAttrs(el: SVGImageElement, data: ImageData, record: ElementRecord): void {
    el.setAttribute('x', String(data.x));
    el.setAttribute('y', String(data.y));
    el.setAttribute('width', String(data.width));
    el.setAttribute('height', String(data.height));
    el.setAttribute('href', data.src);
    if (data.preserveAspectRatio) el.setAttribute('preserveAspectRatio', data.preserveAspectRatio);
    this.setTransform(el, data.transform);
    this.applyCommonAttrs(el, record);
  }
  ...
  private createLinearGradient(record: ElementRecord): SVGLinearGradientElement {
    const data = record.data as LinearGradientData;
  ...
  private updateLinearGradient(el: SVGLinearGradientElement, data: LinearGradientData): void {
    this.applyLinearGradientAttrs(el, data);
  }
   ...
   * @param data 线性渐变数据
   */
  private applyLinearGradientAttrs(el: SVGLinearGradientElement, data: LinearGradientData): void {
    ...
    if (data.gradientUnits) el.setAttribute('gradientUnits', data.gradientUnits);
  ...
  private createRadialGradient(record: ElementRecord): SVGRadialGradientElement {
    const data = record.data as RadialGradientData;
  ...
  private updateRadialGradient(el: SVGRadialGradientElement, data: RadialGradientData): void {
    this.applyRadialGradientAttrs(el, data);
  }
   ...
   * @param data 径向渐变数据
   */
  private applyRadialGradientAttrs(el: SVGRadialGradientElement, data: RadialGradientData): void {
    el.setAttribute('id', data.id);
    el.setAttribute('cx', String(data.cx));
    el.setAttribute('cy', String(data.cy));
    el.setAttribute('r', String(data.r));
    if (data.fx !== undefined) el.setAttribute('fx', String(data.fx));
    if (data.fy !== undefined) el.setAttribute('fy', String(data.fy));
    if (data.gradientUnits) el.setAttribute('gradientUnits', data.gradientUnits);
    while (el.firstChild) el.removeChild(el.firstChild);
    for (const stop of data.stops) {
      const s = document.createElementNS(NS, 'stop');
  ...
  private createClipPathEl(record: ElementRecord, defId: string): SVGClipPathElement {
    const data = record.data as ClipPathData;
  ...
  /** 更新 <clipPath>：重新生成子元素 */
  private updateClipPathEl(el: SVGClipPathElement, data: ClipPathData, defId: string): void {
    el.setAttribute('id', defId);
    while (el.firstChild) el.removeChild(el.firstChild);
    this.fillClipPathChildren(el, data);
  }
   ...
   * @param data 裁剪路径数据
   */
  private fillClipPathChildren(el: SVGClipPathElement, data: ClipPathData): void {
    let childEl: SVGElement | null = null;
   ...
   * @param data 画笔数据
   */
  private setPaintAttrs(
    el: SVGElement,
    data: { fill?: string; stroke?: string; strokeWidth?: number; strokeDasharray?: string },
  ): void {
    ...
    if (data.strokeWidth !== undefined) el.setAttribute('stroke-width', String(data.strokeWidth));
    if (data.strokeDasharray) el.setAttribute('stroke-dasharray', data.strokeDasharray);
   ...
   * @param record 元素记录
   */
  private applyCommonAttrs(el: SVGElement, record: ElementRecord): void {
    const data = record.data as ShapeCommonData;
    if (this.model) {
    ...
    if (data.visible === false) el.setAttribute('visibility', 'hidden');
    else el.removeAttribute('visibility');
    if (data.pointerEvents === 'none') el.setAttribute('pointer-events', 'none');
    else el.removeAttribute('pointer-events');
  }
  ...
  private createFilterEl(record: ElementRecord, defId: string): SVGFilterElement {
    const data = record.data as FilterData;
  ...
  /** 更新 <filter>：重新构建滤镜链 */
  private updateFilterEl(el: SVGFilterElement, data: FilterData, defId: string): void {
    this.applyFilterAttrs(el, data, defId);
  }
   ...
   * @param data 滤镜数据
   */
  private applyFilterAttrs(el: SVGFilterElement, data: FilterData, defId: string): void {
    el.setAttribute('id', defId);
    ...
    for (let i = 0; i < data.effects.length; i++) {
      ...
      switch (effect.type) {
        ...
        case 'opacity': {
          ...
          const funcG = func.cloneNode() as Element;
          feEl.appendChild(funcG);
          const funcB = func.cloneNode() as Element;
        ...
        case 'dropShadow': {
          ...
          feEl.setAttribute('stdDeviation', String(effect.value));
          if (effect.color) feEl.setAttribute('flood-color', effect.color);
          break;
        }
        case 'grayscale': {
  ...
  private createMaskEl(record: ElementRecord, defId: string): SVGMaskElement {
    const data = record.data as MaskData;
  ...
  /** 更新 <mask>：重新设置属性和子元素 */
  private updateMaskEl(el: SVGMaskElement, data: MaskData, defId: string): void {
    this.applyMaskAttrs(el, data, defId);
  }
   ...
   * @param data 遮罩数据
   */
  private applyMaskAttrs(el: SVGMaskElement, data: MaskData, defId: string): void {
    el.setAttribute('id', defId);
   ...
   */
  private setTransform(el: SVGElement, t?: Transform): void {
    if (!t) { el.removeAttribute('transform'); return; }
    const parts: string[] = [];
    ...
    if (parts.length > 0) el.setAttribute('transform', parts.join(' '));
    else el.removeAttribute('transform');
  }
}

export { SVGRenderer, type ShapeCommonData };
