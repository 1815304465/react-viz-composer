 ...
 */
function parseDashArray(dash?: string): number[] | undefined {
  if (!dash) return undefined;
  const parts = dash.split(/[\s,]+/).map(Number).filter((n) => !Number.isNaN(n));
  return parts.length > 0 ? parts : undefined;
}

/** preserveAspectRatio 计算后的 drawImage 目标区域 */
 ...
 */
function computeImageDrawRect(data: ImageData, imgW: number, imgH: number): ImageDrawRect {
  ...
  let dh = data.height;
  if (meetOrSlice === 'meet') {
    if (boxAspect > imgAspect) dw = data.height * imgAspect;
    else dh = data.width / imgAspect;
  } else {
    if (boxAspect > imgAspect) dh = data.width / imgAspect;
    else dw = data.height * imgAspect;
  }
  let dx = data.x;
  let dy = data.y;
  ...
  if (align.includes('Max')) {
    if (align.startsWith('x') || align.includes('YMax')) dx += data.width - dw;
  }
  if (align.includes('YMid')) dy += (data.height - dh) / 2;
  else if (align.includes('YMax')) dy += data.height - dh;
  return { dx, dy, dw, dh };
}

export { parseDashArray, computeImageDrawRect, type ImageDrawRect };
