...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animValue, animSize, useChartItemHover, hoverStrokeWidth, type ChartItemHoverProps, scaleLinear, scaleBand, Axis, Grid, SEMANTIC_6, TEXT_COLOR } from '@react-viz-composer/components';
...
interface Props extends ChartItemHoverProps<PictorialBarItem> {
  ...
  /** 每个图标代表的数量单位，默认 5 */
  unitSize?: number;
  /** 图标 URL（SVG data URI 或普通 URL），默认使用内置的圆形 SVG */
  iconUrl?: string;
  /** 每个图标的尺寸 */
  iconW?: number;
  iconH?: number;
                ...
                {progress > 0.3 && (
                  <>
                    <Rect
                      x={x + 2}
                      y={PLOT_HEIGHT - 4}
                      width={animSize(xScale.bandwidth - 4, progress) > 0 ? xScale.bandwidth - 4 : 0}
                      height={4}
                      fill={SEMANTIC_6[0]}
                      rx={2}
                      ry={2}
