...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animValue, useChartItemHover, hoverStrokeWidth, CATEGORY_12 } from '@react-viz-composer/components';
export function RoseChart(props) {
    ...
    return (_jsx(ChartFrame, { entryDuration: 900, children: (progress) => (_jsxs(_Fragment, { children: [slices.map((s, i) => {
                    ...
                    dParts.push('Z');
                    const d = dParts.join(' ');
                    return (_jsx(Path, { d: d, fill: color, stroke: "#fff", strokeWidth: hoverStrokeWidth(1, hovered), ...bindHover(s) }, s.name));
                }), slices.map((s, i) => {
                    if (progress < 0.5)
                        return null;
                    const ratio = animValue(s.value, progress) / maxVal;
                    ...
                    return (_jsx(Text, { x: lx, y: ly + 4, text: s.name, fontSize: 11, fontFamily: "sans-serif", fill: ratio > 0.5 ? '#fff' : '#595959', textAlign: "middle" }, `t-${s.name}`));
                })] })) }));
}
