...
export function SingleAxisScatterChart(props) {
    const { data, onItemEnter, onItemLeave } = props;
    const { bindHover, isHovering } = useChartItemHover({ onItemEnter, onItemLeave }, (p) => `${p.value}-${p.size}-${p.group}`);
    const points = data ?? [
    ...
    return (_jsx(ChartFrame, { children: (progress) => (_jsxs(_Fragment, { children: [_jsx(Grid, { scale: xScale, orient: "x" }), points.map((p, i) => {
                    ...
                    return (_jsx(Ellipse, { cx: cx, cy: yCenter, rx: hovered ? baseR + 3 : baseR, ry: hovered ? baseR + 3 : baseR, fill: color + '99', stroke: color, strokeWidth: hoverStrokeWidth(1.5, hovered), ...bindHover(p) }, i));
