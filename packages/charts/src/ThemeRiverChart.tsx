...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, useChartItemHover, hoverOpacity, type ChartItemHoverProps, scaleBand, scaleLinear, type BandScale, Axis, Grid, CATEGORY_12, animValue } from '@react-viz-composer/components';
...
export function ThemeRiverChart(props: Props) {
  ...
  const xScale: BandScale = useMemo(
    () => scaleBand(cats, [0, PLOT_WIDTH], 0.3),
    [cats],
  );

  const yScale = useMemo(
    () => scaleLinear([0, maxTotal * 1.1], [PLOT_HEIGHT, 0]),
    [maxTotal],
  );
          ...
          {sers.map((ser, i) => {
            ...
            return (
              <Path
                ...
                d={animD}
                fill={color}
                opacity={hoverOpacity(0.65, hovered)}
                stroke={color}
                strokeWidth={0.5}
                {...bindHover(payload)}
              />
