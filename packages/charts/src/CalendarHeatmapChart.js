...
import { ChartFrame, PLOT_WIDTH, animValue, useChartItemHover, hoverStrokeWidth, TEXT_COLOR } from '@react-viz-composer/components';
/** 绿色梯度 */
...
export function CalendarHeatmapChart(props) {
    ...
    const { bindHover, isHovering } = useChartItemHover({ onItemEnter, onItemLeave }, (p) => p.date);
    // 生成一年的模拟数据（当前年份）
    ...
    const offsetY = 30;
    return (_jsx(ChartFrame, { children: (progress) => (_jsxs(_Fragment, { children: [DAY_LABELS.map((label, i) => (_jsx(Text, { x: offsetX - 8, y: offsetY + i * CELL_STEP + CELL_SIZE / 2 + 3, text: label, fontSize: 10, fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "end" }, `day-${i}`))), (() => {
                    const months = [];
                    for (let m = 0; m < 12; m++) {
                        ...
                        // 去重，只保留第一个出现的
                        if (months.length > 0 && months[months.length - 1].label === `${m + 1}月`)
                            continue;
                    ...
                    return months.map((mo) => {
                        ...
                        return (_jsx(Text, { x: x, y: offsetY - 10, text: mo.label, fontSize: 9, fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "middle" }, `mo-${mo.label}`));
                    });
                })(), grid.map((week, wi) => week.map((day) => {
                    ...
                    return (_jsx(Rect, { x: x, y: y, width: CELL_SIZE, height: CELL_SIZE, fill: heatColor(av, maxVal), stroke: "#fff", strokeWidth: hoverStrokeWidth(0.5, hovered), rx: 3, ry: 3, ...bindHover(payload) }, day.date));
                }))] })) }));
}
