...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animSize, useChartItemHover, hoverStrokeWidth, type ChartItemHoverProps, scaleBand, scaleLinear, Axis, Grid, SEMANTIC_6 } from '@react-viz-composer/components';
...
export function BidirectionalBarChart(props: Props) {
  ...
  const maxV = Math.max(
    ...dataset.map((d) => Math.max(d.positive, d.negative)),
  ) * 1.1;
          ...
          <Fragment key={`label-${d.name}`}>
            ...
            {negH > 10 && (
              <Text
                x={x + xScale.bandwidth / 2}
                y={zeroY + negH + 12}
                text={String(Math.round(d.negative * progress))}
                fontSize={10}
                fontFamily="sans-serif"
                fill={negColor}
                textAlign="middle"
