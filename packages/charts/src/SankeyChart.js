...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animValue, animSize, useChartItemHover, hoverStrokeWidth, CATEGORY_12 } from '@react-viz-composer/components';
export function SankeyChart(props) {
    const { nodes, links, onItemEnter, onItemLeave } = props;
    const { bindHover, isHovering } = useChartItemHover({ onItemEnter, onItemLeave }, (n) => n.name);
    const ns = nodes ?? [
        { name: '太阳能', depth: 0 },
        { name: '风能', depth: 0 },
        { name: '水电', depth: 0 },
    ...
    const nodeH = 26;
    const xOfDepth = (d) => d * (colW + colGap);
    const colNodes = {};
    ns.forEach((n) => {
        if (!colNodes[n.depth])
            colNodes[n.depth] = [];
    ...
    const inValue = (name) => lks.filter((l) => l.target === name).reduce((s, l) => s + l.value, 0);
    const outValue = (name) => lks.filter((l) => l.source === name).reduce((s, l) => s + l.value, 0);
    return (_jsx(ChartFrame, { entryDuration: 900, children: (progress) => (_jsxs(_Fragment, { children: [lks.map((l, i) => {
                    ...
                    return (_jsx(Path, { d: d, fill: "none", stroke: CATEGORY_12[i % CATEGORY_12.length] + '60', strokeWidth: Math.max(1, animValue(l.value, progress) / 5) }, `l-${i}`));
                }), ns.map((n, i) => {
                    ...
                    return (_jsx(Rect, { x: x, y: y, width: animSize(colW, progress), height: animSize(nodeH, progress), fill: color, stroke: color, strokeWidth: hoverStrokeWidth(1, hovered), ...bindHover(payload) }, n.name));
                }), ns.map((n) => {
                    const x = xOfDepth(n.depth);
                    const y = nodeY[n.name];
                    if (progress < 0.4)
                        return null;
                    return (_jsx(Text, { x: x + colW / 2, y: y + nodeH / 2 + 4, text: n.name, fontSize: 11, fontFamily: "sans-serif", fill: "#fff", textAlign: "middle" }, `t-${n.name}`));
                })] })) }));
}
