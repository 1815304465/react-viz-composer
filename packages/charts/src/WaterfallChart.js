...
import { Rect, Line, Text } from '@react-viz-composer/core';
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animSize, animValue, useChartItemHover, hoverStrokeWidth, scaleLinear, scaleBand, Axis, Grid, TEXT_COLOR } from '@react-viz-composer/components';
const GREEN = '#52c41a';
const RED = '#f5222d';
export function WaterfallChart(props) {
    ...
    const yScale = useMemo(() => scaleLinear([yMin, yMax], [PLOT_HEIGHT, 0]), [yMin, yMax]);
    return (_jsx(ChartFrame, { children: (progress) => (_jsxs(_Fragment, { children: [_jsx(Grid, { scale: yScale, orient: "y" }), ranges.map((r, i) => {
                    if (i === 0)
                        return null;
                    ...
                    return (_jsx(Line, { points: [
                            ...
                            { x: curX, y: curY },
                        ], stroke: "#bfbfbf", strokeWidth: 1 }, `link-${r.name}`));
                }), ranges.map((r) => {
                    ...
                    return (_jsx(Rect, { x: x, y: barY, width: xScale.bandwidth, height: animSize(barH, 1) > 0 ? barH : 0, fill: color, stroke: color, strokeWidth: hoverStrokeWidth(1, hovered), ...bindHover(r) }, r.name));
                }), ranges.map((r) => {
                    const x = xScale(r.name);
                    const endY = yScale(animValue(r.end, progress));
                    ...
                    return (_jsx(Text, { x: x + xScale.bandwidth / 2, y: endY + offset, text: String(labelValue), fontSize: 11, fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "middle" }, `t-${r.name}`));
