...
export function CurvatureCombChart(props) {
    const { data, toothScale = 30, sampleRate = 4 } = props;
    const points = data ?? defaultCurveData();
    // 计算 x/y 范围
    ...
    const curveD = points
        ...
        .join(' ');
    return (_jsx(ChartFrame, { entryDuration: 1000, children: (progress) => (_jsxs(_Fragment, { children: [_jsx(Grid, { scale: xScale, orient: "x" }), _jsx(Grid, { scale: yScale, orient: "y" }), combTeeth.map((tooth, i) => {
                    ...
                    return (_jsxs(Fragment, { children: [_jsx(Line, { points: [
                                ...
                                ], stroke: SEMANTIC_6[3], strokeWidth: 0.8 }), progress > 0.4 && (_jsx(Ellipse, { cx: xScale(tooth.tx), cy: yScale(tooth.ty), rx: 2, ry: 2, fill: SEMANTIC_6[3], stroke: "none" }))] }, i));
                }), _jsx(Path, { d: curveD, fill: "none", stroke: SEMANTIC_6[0], strokeWidth: 2 }), combTeeth.map((tooth, i) => (_jsx(Ellipse, { cx: xScale(tooth.x), cy: yScale(tooth.y), rx: 3, ry: 3, fill: "#fff", stroke: SEMANTIC_6[0], strokeWidth: 1.5 }, `sample-${i}`))), _jsx(Axis, { scale: xScale, orient: "bottom" }), _jsx(Axis, { scale: yScale, orient: "left" })] })) }));
}
function defaultCurveData() {
    const pts = [];
    for (let i = 0; i < 80; i++) {
        const t = i / 79;
        const x = 20 + t * 80;
