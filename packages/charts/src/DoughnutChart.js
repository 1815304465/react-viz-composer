...
export function DoughnutChart(props) {
    ...
    return (_jsx(ChartFrame, { background: "#fff", entryDuration: 900, children: (progress) => {
            ...
            return (_jsxs(_Fragment, { children: [arcs.map((a, i) => {
                    ...
                    }), slices.map((s, i) => (_jsx(Text, { x: CHART_WIDTH - 140 + 18, y: 30 + i * 24 + 2, text: s.name, fontSize: 12, fontFamily: "sans-serif", fill: TEXT_COLOR }, `leg-t-${s.name}`))), progress >= 0.6 && (_jsx(Text, { x: cx, y: cy + 4, text: "\u603B\u6570", fontSize: 14, fontFamily: "sans-serif", fill: "#595959", textAlign: "middle" })), progress >= 0.6 && (_jsx(Text, { x: cx, y: cy + 22, text: String(Math.round(total * progress)), fontSize: 18, fontWeight: "bold", fontFamily: "sans-serif", fill: "#262626", textAlign: "middle" }))] }));
        } }));
}
