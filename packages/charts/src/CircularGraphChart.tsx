...
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animSize, useChartItemHover, hoverStrokeWidth, type ChartItemHoverProps, CATEGORY_12, TEXT_COLOR } from '@react-viz-composer/components';
  ...
  return (
    <ChartFrame>
      {(progress) => {
        ...
        // 带动画的节点位置
        const animatedPos = new Map<string, { x: number; y: number }>();
            ...
            {edges.map((edge, i) => {
              ...
              return (
                <Fragment key={`edge-${i}`}>
                  ...
                  />
                  {/* 箭头 */}
                  {drawArrowHead(endX, endY, dx / dist, dy / dist, 6)}
                </Fragment>
              );
            })}
              ...
              return (
                <Fragment key={node.id}>
                  <Ellipse
                    cx={pos.x}
                    cy={pos.y}
                    rx={hovered ? nodeR + 3 : nodeR}
                    ry={hovered ? nodeR + 3 : nodeR}
                    fill={color}
                    stroke="#fff"
                  ...
                  <Text
                    x={pos.x}
                    y={pos.y + animSize(nodeR + 16, progress)}
                    text={node.label}
                    fontSize={10}
...
function drawArrowHead(
  ...
  return <Path d={d} fill={CATEGORY_12[5]} opacity={0.6} />;
}
