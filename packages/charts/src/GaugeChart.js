...
export function GaugeChart(props) {
    ...
    return (_jsx(ChartFrame, { entryDuration: 900, children: (progress) => (_jsx(GaugeInner, { cx: cx, cy: cy, outerR: outerR, min: min, max: max, value: value, valueRatio: valueRatio, progress: progress, title: title })) }));
}
/** 内部组件：持有持续动画状态 */
function GaugeInner({ cx, cy, outerR, min, max, value, valueRatio, progress, title, }) {
    ...
    function arcPath(r, fromRatio, toRatio) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Path, { d: arcPath(arcCenterR, 0, 1), fill: "none", stroke: "#f0f0f0", strokeWidth: outerR - innerR }), effectiveRatio > 0 && (_jsx(Path, { d: arcPath(arcCenterR, 0, effectiveRatio), fill: "none", stroke: SEMANTIC_6[0], strokeWidth: outerR - innerR })), [0, 0.25, 0.5, 0.75, 1].map((r) => {
                ...
                return (_jsx(Line, { points: [{ x: x1, y: y1 }, { x: x2, y: y2 }], stroke: AXIS_COLOR, strokeWidth: 3 }, `tick-${r}`));
                ...
                return (_jsx(Text, { x: lx, y: ly, text: String(Math.round(labelVal)), fontSize: 11, fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "middle" }, `tl-${r}`));
            }), _jsx(Line, { points: [{ x: cx, y: cy }, { x: px, y: py }], stroke: SEMANTIC_6[3], strokeWidth: 3 }), _jsx(Ellipse, { cx: cx, cy: cy, rx: 8, ry: 8, fill: SEMANTIC_6[3] }), _jsx(Text, { x: cx, y: cy - 8, text: String(displayValue), fontSize: 28, fontWeight: "bold", fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "middle" }), _jsx(Text, { x: cx, y: cy + 16, text: `/ ${max}`, fontSize: 12, fontFamily: "sans-serif", fill: TEXT_COLOR, textAlign: "middle" }), title && (_jsx(Text, { x: cx, y: cy + 36, text: title, fontSize: 11, fontFamily: "sans-serif", fill: TEXT_COLOR, opacity: 0.6, textAlign: "middle" }))] }));
}
