...
export function StackedAreaChart(props) {
    ...
    return (_jsx(ChartFrame, { children: (progress) => (_jsxs(_Fragment, { children: [_jsx(Grid, { scale: yScale, orient: "y" }), series.map((s, si) => {
                    const color = SEMANTIC_6[si % SEMANTIC_6.length];
                    const points = s.values.map((v, i) => {
                        const cumulativeBelow = series
                            .slice(0, si)
                            .reduce((sum, prev) => sum + prev.values[i], 0);
                        const bottomY = yScale(animValue(cumulativeBelow, progress));
                        const topY = yScale(animValue(cumulativeBelow + v, progress));
                        return {
                    ...
                    const areaD = `M ${points[0].x} ${points[0].yBottom} ` +
                        points.map((p) => `L ${p.x} ${p.yTop}`).join(' ') +
                        ` L ${points[points.length - 1].x} ${points[points.length - 1].yBottom} ` +
                        [...points].reverse().map((p) => `L ${p.x} ${p.yBottom}`).join(' ') +
                        ' Z';
                    return (_jsx(Path, { d: areaD, fill: color + '50', stroke: "none" }, `area-${s.name}`));
                }), series.map((s, si) => {
                    const color = SEMANTIC_6[si % SEMANTIC_6.length];
                    const points = s.values.map((v, i) => {
                        const cumulativeBelow = series
                    ...
                    return s.values.map((v, i) => {
                        ...
                        return (_jsx(Ellipse, { cx: xScale(cats[i]) + xScale.bandwidth / 2, cy: topY, rx: hovered ? 6 : 4, ry: hovered ? 6 : 4, fill: "#fff", stroke: color, strokeWidth: hoverStrokeWidth(2, hovered), ...bindHover(payload) }, `${s.name}-${i}`));
                    });
                }), series.map((s, si) => {
                    const color = SEMANTIC_6[si % SEMANTIC_6.length];
                    const points = s.values.map((v, i) => {
