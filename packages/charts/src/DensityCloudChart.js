 ...
 */
import { Rect, Ellipse } from '@react-viz-composer/core';
import { ChartFrame, PLOT_WIDTH, PLOT_HEIGHT, animValue, scaleLinear, Axis, Grid } from '@react-viz-composer/components';
/** 颜色映射：蓝→青→绿→黄→红 */
...
export function DensityCloudChart(props) {
    const { data, bandwidth, gridSize = 40 } = props;
    const points = data ?? defaultDensityData();
    // 数据范围
    ...
    return (_jsx(ChartFrame, { children: (progress) => (_jsxs(_Fragment, { children: [_jsx(Grid, { scale: xScale, orient: "x" }), _jsx(Grid, { scale: yScale, orient: "y" }), cells.map((row, ri) => row.map((d, ci) => {
                    const av = animValue(d * scale, progress);
                    if (av < 0.01)
                        return null;
                    return (_jsx(Rect, { x: ci * cellW, y: ri * cellH, width: cellW, height: cellH, fill: cloudColor(av), stroke: "none" }, `c-${ri}-${ci}`));
                })), points.map((p, i) => {
                    const px = xScale(p.x);
                    const py = yScale(p.y);
                    return (_jsx(Ellipse, { cx: px, cy: py, rx: 2, ry: 2, fill: "rgba(255,255,255,0.3)", stroke: "rgba(255,255,255,0.6)", strokeWidth: 0.5 }, `p-${i}`));
                }), _jsx(Axis, { scale: xScale, orient: "bottom" }), _jsx(Axis, { scale: yScale, orient: "left" })] })) }));
}
function defaultDensityData() {
    const out = [];
    // 中心密集 + 外围稀疏
    for (let i = 0; i < 200; i++) {
