...
export function LiquidFillChart(props: Props) {
  ...
  return (
    <ChartFrame background="#fff" entryDuration={1200}>
      {(progress) => (
        <WaterBallInner
          cx={cx} cy={cy} r={r} pct={pct} progress={progress}
        />
      )}
    </ChartFrame>
  );
...
/** 内部组件：持有持续动画状态 */
function WaterBallInner({
  cx, cy, r, pct, progress,
}: {
  cx: number; cy: number; r: number; pct: number; progress: number;
}) {
  ...
  return (
    <>
      <ClipPath id="water-clip" shapeType="ellipse" shapeData={{ cx, cy, rx: r, ry: r }} />
      ...
      {progress > 0.02 && (
        <Path d={fullWaterD} fill={SEMANTIC_6[0]} opacity={0.6} clipPath="url(#water-clip)" />
      )}
