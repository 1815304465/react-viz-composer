...
type VizShapeEventPropName = typeof VIZ_SHAPE_EVENT_PROP_NAMES[number];

/** 使用 VizDragEventHandler 的 prop（而非 VizEventHandler） */
type VizShapeDragEventPropName = 'onDrag' | 'onDragEnd';

/** React 事件 prop → EventSystem 内部 VizEventType 映射 */
const REACT_EVENT_TO_VIZ_TYPE: Partial<Record<VizShapeEventPropName, VizEventType>> = {
  onClick: 'click',
...
const MOUSE_DOWN_EVENT_PROPS: VizShapeEventPropName[] = [
...
type ShapeEventProps = {
  [K in VizShapeEventPropName]?: K extends VizShapeDragEventPropName
    ? VizDragEventHandler
    : VizEventHandler;
};
...
function combineHandlers(
  ...handlers: (VizEventHandler | undefined)[]
): VizEventHandler | undefined {
  const active = handlers.filter(Boolean) as VizEventHandler[];
...
function hasVizDragHandlers(eventProps: ShapeEventProps): boolean {
  return !!(eventProps.onDrag || eventProps.onDragEnd || eventProps.onDragStart);
}

/** 判断是否需要绑定 mousedown 入口 */
function needsMouseDownBinding(eventProps: ShapeEventProps): boolean {
  return hasVizDragHandlers(eventProps)
    || MOUSE_DOWN_EVENT_PROPS.some((key) => eventProps[key]);
}

/** 从 props 中提取事件 handler */
function pickShapeEventProps(props: Record<string, unknown>): ShapeEventProps {
  return pick(props, VIZ_SHAPE_EVENT_PROP_NAMES) as ShapeEventProps;
}

/** 将 React 风格事件 props 转为 EventSystem 标准事件表 */
function buildShapeEvents(
  eventProps: ShapeEventProps,
  handleMouseDown?: VizEventHandler,
): Partial<Record<VizEventType, VizEventHandler>> {
  const events: Partial<Record<VizEventType, VizEventHandler>> = {};
  const buckets = new Map<VizEventType, VizEventHandler[]>();
  ...
  for (const reactName of VIZ_SHAPE_EVENT_PROP_NAMES) {
    ...
    if (handler) {
      ...
      list.push(handler as VizEventHandler);
...
export {
  ...
  REACT_EVENT_TO_VIZ_TYPE,
  MOUSE_DOWN_EVENT_PROPS,
  hasVizDragHandlers,
  needsMouseDownBinding,
  pickShapeEventProps,
  buildShapeEvents,
  NOOP as SHAPE_EVENT_NOOP,
};
export type {
  ShapeEventProps,
  VizShapeEventPropName,
  VizShapeDragEventPropName,
};
