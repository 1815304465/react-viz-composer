...
function CurvatureCombChartPlot(props: Props) {
  ...
  const xScale = scaleLinear([xMin - padX, xMax + padX], [30, plotWidth - 10]);
  const yScale = scaleLinear([yMin - padY, yMax + padY], [plotHeight - 20, 20]);
  ...
  for (let i = 0; i < points.length - 1; i += sampleRate) {
    ...
    combTeeth.push({ x: curr.x, y: curr.y, tx: curr.x + normX * toothLen, ty: curr.y + normY * toothLen, curvature });
  }
  ...
  return (
    <>
      ...
      <Animation playbook={[...TOOTH_PLAYBOOK]}>
        {combTeeth.flatMap((tooth, i) => [
          <Line
            key={`tooth-${i}`}
            points={[
              { x: xScale(tooth.x), y: yScale(tooth.y) },
              { x: xScale(tooth.tx), y: yScale(tooth.ty) },
            ]}
            stroke={SEMANTIC_6[3]}
            strokeWidth={0.8}
            opacity={1}
